...
public class ProjectController {
    ...
    @RequestMapping(value = "/QRStatusUpload", method = RequestMethod.POST)
    public Result QRStatusUpload(@RequestParam(value = "file", required = false) MultipartFile file, HttpServletRequest request) {
        String url = iProjectService.QRStatusUpload(file, request);
        return ResultUtil.success(url);
    }

    @PostMapping("checkoutProject")
    @ApiOperation("切换项目")
    @ApiImplicitParam(name = "projectId", value = "项目ID", dataType = "string", required = true)
    public Result updateProject(HttpServletRequest request,String projectId){
        try {
            if (projectId!=null){
